...
export interface Props extends ComponentProps<'button'> {
  ...
  color?: 'gradient-blue' | 'gray' | 'transparent';
  fullWidth?: boolean;
...
const Button = forwardRef<HTMLButtonElement, PropsWithChildren<Props>>(
  (
    { children, className, contentClassName, variant = 'contained', color = 'gradient-blue', disabled = false, fullWidth = false, loading = false, icon, startIcon, endIcon, ...props },
    _forwardRef
  ) => {
    return createElement(
      props.href ? 'a' : 'button',
      {
        className: cx(`button button--${variant} button--${color}`, fullWidth && 'button--fullWidth', loading === true && 'is-loading', disabled && 'is-disabled', className),
        ref: _forwardRef,
