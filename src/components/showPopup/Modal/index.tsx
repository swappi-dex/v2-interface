...
const Modal: React.FC<{
...
}> = memo(({ Content, title, subTitle, className, onClose }) => {
  ...
  return (
    <BorderBox
      ...
      className={cx(
        "relative w-90vw max-w-400px px-16px py-24px rounded-24px overflow-hidden shadow-popper",
        className
      )}
    >
      ...
      {subTitle && (
        <div className="px-8px text-14px font-medium leading-18px font-not-italic color-gray-normal mt-1">
          {subTitle}
        </div>
      )}
